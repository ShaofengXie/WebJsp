...
/**
 * @Component("boyAnnotation")给组件起个名字
 * 
 * @Autowired 注入属性，可以放在属性或者Setter方法上面byType自动注入
 * @Resource 注入属性，可以放在属性或者Setter方法上面
 *           属性name解析bean的名字，type解析bean的类型，byName自动注入 @Resource(name="usaGirlFirend")、 @Resource(type=usaGirlFirend.class)
 * @Repository dao层bean(持久化层)
 * @Service service层bean(服务层)
 * @Controller mvc控制层bean(表现层) @Scope("session")bean的作用域 singleton 单例，共享bean
 *             prototype 注入新的bean request http request 有效(一个请求一个新的bean) session
 *             http session 有效(一个请求一个新的bean) global session portlet web 有效
 *             全局session
...
public class BoyAnnotation {
	...
	public void setUsaGirlFirend(UsaGirlFriend usaGirlFriend) {
